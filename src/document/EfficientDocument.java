...
public class EfficientDocument extends Document {
	...
	//	System.out.println();
		...
		for (String temp : tokens ){
			...
			if (isWord(temp)) {
				numWords ++;
				numSyllables += countSyllables(temp);
				//this is for special case where sentence ends without proper punctuation
	...
	//	System.out.println("number of sentences so far = " + numSentences);
	//	System.out.println("number of words so far = " + numWords);
	//	System.out.println("number of syllables so far = " + numSyllables);
	//	getNumWords(numWords);
		
		/*
		for (String temp : tokens){
			int aNum = temp.indexOf('.') + temp.indexOf('?') + temp.indexOf('!');
			if (aNum > -3) {
		    //try to figure out why this did not work later
		   //	if (temp.matches(".")); {
				numSentences ++;
			}
			else {
				numWords ++;
				//lets get the syllables while we are here
				numSyllables += countSyllables(temp);
			}
		}
		System.out.println("number of sentences so far = " + numSentences);
